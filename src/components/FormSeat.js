...
export default function FormSeat({session, selectedSeats}) {
    ...
    function storageInformation() {
        let filteredSeats = session.seats.filter((seat)=>selectedSeats.includes(seat.id))
    ...
    function reserveSeat(event) {
        ...
        if(selectedSeats.length === 0 ) {         // check if user selected at least 1 seat
        ...
        axios.post(`${URL}/seats/book-many`, objectToPost)
